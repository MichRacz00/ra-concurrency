...
void ModelAction::print() const
{
	...
	if (file.is_open()) {
		...
		int id2in = id_to_int(tid);
		int retVal = get_return_value();	
		model_print("%-4d %-2d   %-14s  %7s  %14p   %-#18" PRIx64,
								seq_number, id2in, type_str, mo_str, location, retVal);
		file << seq_number << "," << id2in << "," << type_str << "," << mo_str << "," << location << "," << retVal;
		if (is_read()) {
			if (is_write()) {
				int wVal = get_write_value();
				model_print("(%" PRIx64 ")", wVal);
				file << "," << wVal;
			} else {
				file << ",?";
			}
			if (reads_from){
				int rVal = reads_from->get_seq_number();
				model_print("  %-3d", rVal);
				file << "," << rVal;
			}
			else {
		...
		model_print("Error writing to file\n");
	}
}
